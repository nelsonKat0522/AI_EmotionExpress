...
from emoexpress.emotion_classifier import (load_emotion_resources)
from emoexpress.topic_classifier import (load_topic_resources)
from emoexpress.rag import (load_vector_store)
from emoexpress.pipeline import (run_emoexpress_pipeline)
from emoexpress.history_manager import (add_history_record)
        ...
        try:
            with st.spinner(
                ...
                result = run_emoexpress_pipeline(user_story=user_story,
                                            ...
                                            topic_tokenizer=resources["topic_tokenizer"],
                                            topic_model=resources["topic_model"],
            ...
            with topic_column:
                ...
                topic_display = (topic_result["topic"].replace("_", " ").title())
            ...
            st.write(generated_response["empathetic_response"])

            st.subheader("Practical Recommendations")

            for index, recommendation in enumerate(generated_response["recommendations"],start=1):
                st.markdown(f"**{index}.** {recommendation['recommendation']}")
                ...
                if source_title:
                    st.caption(f"Source: {source_title}, page {page}")
            ...
            final_image_path = (image_result.get("final_path") or image_result.get("path"))
            ...
            with st.expander("Knowledge Sources"):
                ...
                if sources:
                    ...
                    available_columns = [column for column in ["rank",
                                                                "title",
                                                                "page",
                                                                "topic",
            ...
            with st.expander("Pipeline Details"):
                st.write("Retrieval strategy:",result["retrieval"]["strategy"])
